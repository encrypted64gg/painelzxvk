...
def show_banner(text='ZXVK', font='standard'):
...
def memory_optimizer(threshold=80, interval=5):
    while True:
        ...
        if used >= threshold:
            ...
            if os.name != 'nt':
                try:
                    ...
                    print(Fore.RED + "[RAM OPT] Sem permissão para drop_caches (sudo requerido).")
                except Exception as e:
                    print(Fore.RED + f"[RAM OPT] Erro ao limpar caches: {e}")
            print(Fore.YELLOW + f"[RAM OPT] Coleta de lixo forçada. Uso atual: {used:.1f}%")
        time.sleep(interval)
...
# 11: scan de ports abertas em um ip
def scan_ports():
    target = input(Fore.YELLOW + "Alvo (IP ou domínio): ").strip()
    try:
        host = socket.gethostbyname(target)
    except socket.gaierror:
        print(Fore.RED + f"Não foi possível resolver o host: {target}")
        return

    start_port = int(input(Fore.YELLOW + "Porta inicial: "))
    end_port   = int(input(Fore.YELLOW + "Porta final: "))
    thread_count = int(input(Fore.YELLOW + "Número de threads: "))

    print(Fore.CYAN + f"Iniciando scan em {host} de {start_port} até {end_port}...\n")
        ...
        while not port_queue.empty():
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                if s.connect_ex((host, port)) == 0:
                    print(Fore.GREEN + f"[OPEN] Porta {port}")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(Fore.YELLOW + "11." + Style.RESET_ALL + " Scan de portas abertas em um IP/HOST (Roteador)")
        print(Fore.YELLOW + "12." + Style.RESET_ALL + " Sair")
        choice = input(Fore.BLUE + "Opção: ").strip()
