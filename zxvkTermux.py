...
import json,socket,threading,time,random,hashlib,getpass,string,pyfiglet,secrets,requests,subprocess,re
...
def save_queries(data):
	...
	with open(DB_PATH,'w',encoding=_B)as A:json.dump(data,A,ensure_ascii=_E,indent=2)
...
def save_users(users):
	with open(USERS_FILE,'w',encoding=_B)as A:json.dump(users,A,ensure_ascii=_E,indent=2)
...
def create_initial_user():
	...
	if not A:
		...
		while _A:
			B=input(Fore.CYAN+'Novo usuário (admin): ').strip();C=getpass.getpass(_F).strip();D=getpass.getpass('Confirme a senha: ').strip()
...
def authenticate():
	A=load_users()
	for D in range(3):
		B=input(Fore.CYAN+'Usuário: ').strip();C=getpass.getpass(_F).strip()
...
def show_banner(text='ZXVK',font='ansi_shadow'):A=pyfiglet.figlet_format(text,font=font);print(Fore.GREEN+center_text('Executado com sucesso.'));print(Fore.CYAN+center_text('Sistema de segurança ativado.')+_C);print(Fore.CYAN+center_text(A)+_C)
def loading_bar(steps=50,delay=.1):
	A=steps;C=os.get_terminal_size().columns
	for B in range(A+1):D=B;E=A-B;F=int(B/A*100);G='#'*D+'-'*E;H=f"[{G}] {F:3d}%";print(Fore.CYAN+H.center(C),end='\r');time.sleep(delay)
	print()
def show_credits():A='By encrypted64 | Discord: https://discord.gg/993tCR8F';print(Style.DIM+Fore.YELLOW+center_text(A))
def manage_database():
	A=load_data()
	while _A:
		print(Fore.BLUE+'--- Gerenciar Database ---');print(Fore.YELLOW+'1.'+Style.RESET_ALL+' Adicionar dado');print(Fore.YELLOW+'2.'+Style.RESET_ALL+' Mostrar dados');print(Fore.YELLOW+'3.'+Style.RESET_ALL+' Gerar keys criptografadas');print(Fore.YELLOW+'4.'+Style.RESET_ALL+' Voltar');B=input(Fore.BLUE+_G).strip()
		if B=='1':
			...
			if A:
				...
				for D in A:print(_D+D)
			else:print(Fore.RED+_H)
		elif B=='3':
...
def filter_ips():
	...
	try:
		E=socket.getaddrinfo(D,None);A=sorted({A[4][0]for A in E})
		if A:
			print(Fore.GREEN+'IPs encontrados:')
			for F in A:print(_D+F)
...
def ip_tracker():
	...
	if A:
		...
		for B in A:print(_D+B)
	else:print(Fore.RED+_H)
def dos_test():
	B=input(Fore.YELLOW+'Alvo (IP ou domínio): ').strip();C=int(input(Fore.YELLOW+'Porta: '));A=int(input(Fore.YELLOW+'Threads: '));print(Fore.CYAN+'Iniciando DoS local... (CTRL+C para parar)')
	def D():
		while _A:
...
def flood_site():
	A=input(Fore.YELLOW+_J).strip();B=int(input(Fore.YELLOW+'Threads simultâneas: '));print(Fore.CYAN+'Iniciando flood... (CTRL+C para parar)')
	def C():
		while _A:
...
def vuln_scan():
	A=input(Fore.YELLOW+_J).strip();print(Fore.CYAN+'Iniciando verificação de vulnerabilidades...')
	try:
...
if __name__=='__main__':
	...
	while _A:
		...
		elif choice=='11':print(Fore.GREEN+'Saindo...');loading_bar();show_credits();show_banner();break
		else:print(Fore.RED+_I)
